/**
   Raduino_diag v3 for BITX40 - Allard Munters PE1NWL (pe1nwl@gooddx.net)
...
// USB/LSB parameters
#define CAL_VALUE 1575               // VFO calibration value (180 ppm)
...
void setFrequency() {
  ...
  DEBUG_PRINT(analogRead(ANALOG_TUNING));
  DEBUG_PRINT(", CLAR pot ");
  DEBUG_PRINTLN(analogRead(CLARIFIER) - 512);
  updateDisplay();
}
...
void doTuning() {
  ...
  // the tuning knob is at neither extremities, tune the signals as usual
    ...
    if (abs(knob - old_knob) > 4 || abs(clar - clar_old) > 1) { // improved "flutter fix": only change frequency when the current knob position is more than 4 steps away from the previous position
